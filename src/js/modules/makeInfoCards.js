  ...
  stations.forEach(station => {
    ...
    bikes.forEach(bike => {
      let nameZh = station.StationName.Zh_tw,
          ...
          adZh = station.StationAddress.Zh_tw,
          adEn = station.StationAddress.En,
          time = station.SrcUpdateTime;

      let status = bike.ServiceStatus,
          id = bike.StationID,
          rentNum = bike.AvailableRentBikes,
          returnNum = bike.AvailableReturnBikes,
          bikeNum = '';
      ...
      cards += `
        <div data-id="${id}" class="card shadow pt-3 pb-4 px-4 visually-hidden" data-item="card-item">
          <div class="card-body p-0 fs-12 text-primary" data-item="card-item">
            ${status}
            <h5 class="card-title fw-700 fs-20 mb-1" data-item="card-item">${nameZh}</h5>
            <h6 class="card-subtitle mb-2 text-info mb-3 pb-1" data-item="card-item">${nameEn}</h6>
            <p class="card-text mb-0 fw-500" data-item="card-item">${adZh}</p>
            <p class="card-text text-info mb-3 pb-1" data-item="card-item">${adEn}</p>
            <p class="card-text text-info fw-400" data-item="card-item">更新時間︰${time}</p>
